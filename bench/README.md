...
| `../ab/results.csv` | the author's runs of `tools/channel_cable.py` with VARA HF and Aether at both ends (P9-1) | the **A/B bench**: goodput per profile and SNR for both modems through the identical real-time channel at the audio level; the protocol is `bench/ab/README.md`, the table `tools/ab_summary.py` |
