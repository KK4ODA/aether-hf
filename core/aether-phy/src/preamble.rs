...
impl Preamble {
    ...
    pub fn new(params: WaveformParams) -> Self {
        ...
        let sequences = [
            this.sc_values(FrameType::Data),
            this.sc_values(FrameType::Control),
        ];
    ...
    pub fn symbols_for(&self, header: &FrameHeader, layout: &FrameLayout) -> Vec<Vec<Complex>> {
        vec![self.sc_values(header.frame_type); layout.preamble_symbols]
    }
...
mod tests {
    ...
    fn every_chip_sequence_is_unit_magnitude_and_well_separated() {
        ...
        // the narrow waveform: 32 chips, fifty-two sequences, a looser bound
