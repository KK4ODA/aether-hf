...
answer-only station makes none of — a `connect`, `probe`, `beacon`, `beacon.every`, `test.start` or
`datagram.send` — fails with code `refused`, not retryable: the setting refuses it, and waiting
will not change that. `not_idle` (retryable) and `already_connected` are for what a session, a
probe or a test under way holds up.
...
mode number means nothing without the `bandwidth_hz` it came with. `[radio] bandwidth` is the station's own waveform
and is live: a change moves the station once it is idle (ADR-0026), as `bandwidth.set` and a
narrower call do; `[radio] answer_only` (live) makes the station take calls and make none: `connect`,
`beacon`, `beacon.every`, `probe`, `test.start` and `datagram.send` are refused (`refused`, §3)
while it is set, and probes are still answered. It does not decide what the rules allow — `[regulatory] control`
...
| Method | Params | Result |
|---|---|---|
| `connect` | `remote`, `callsign?` | `session` (the id); then `state` events. `callsign` picks which of the station's callsigns to call as (the first, when absent). The call goes out in the bandwidth the station runs now (`status.bandwidth`). Refused `bad_params` (no `remote`; a `callsign` not the station's), `already_connected` (a session is up, or a call is going out), `refused` (the station is answer-only), `regulatory` |
...
| `listen` | `enabled` | `{"enabled": true}` for true; `false` is refused `unsupported`: with no host program attached the station answers every call to its callsigns. A host program's `LISTEN OFF` is what keeps it from answering (`status.answering`) |
| `bandwidth.set` | `hz` (500, 2300, 2750 — which is 2300 — or `null`) | `status.bandwidth` as it now stands. The bandwidth a host program asked for with VARA's `BW500`, `BW2300` or `BW2750` (the VARA-compatible adapter's client call); `null` goes back to the station's own. The station moves at once when idle — rebuilding its receiver, filters and busy detector and running the other ladder — and the move is a `bandwidth` event and a `log` event named `bandwidth`. Refused `refused` (retryable) while a session, a call, a probe, a Test or a transmission is under way — the station stays, and the reason says why; `bad_params` for another number. The request holds while the host program is attached; when it goes the station goes back to its own once idle (ADR-0026) |
