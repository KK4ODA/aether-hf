...
| Method | Params | Result |
|---|---|---|
| `connect` | `remote`, `callsign?` | `session` (the id); then `state` events. `callsign` picks which of the station's callsigns to call as (the first, when absent). The bandwidth is the station's (`[radio] bandwidth`), never a call's. Refused `bad_params` (no `remote`; a `callsign` not the station's), `already_connected` (a session is up, or a call is going out), `refused` (the station is answer-only), `regulatory` |
