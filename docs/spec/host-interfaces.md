...
| `CQFRAME <source> <bandwidth>` | Sends a beacon, addressed to nobody, on the tone floor (the control API's `beacon`), under the name the host gave it when this station's callsign is its base — VarAC's `CQFRAME KK4ODA-9 500`, whose suffix means something to the program at the other end — and under the station's own callsign otherwise (ADR-0024) | `OK` once understood; a beacon the modem refuses — in a session, on an answer-only station, or by the rules (ADR-0018) — is not sent, and nothing more is said |
...
| `CQFRAME <call> <bandwidth>` | A beacon was heard: to a VARA host a CQ frame, which is how a chat program lists who is on (VarAC's beacons and CQs). It follows the `SN` line that gives the beacon's strength, and carries the name as it was sent and the bandwidth the beacon says its sender runs (ADR-0024) — this station's, for a beacon from before that |
...
| Client | Status |
|---|---|
| The test suite's own host client | **Passing** — setup, call, session notifications, payload both ways, second-host refusal, a `DISCONNECTED` for every call that ends without a session (unanswered, `ABORT`, `DISCONNECT`) and one only for a call aborted and placed again at once; and `a_host_hears_how_each_of_its_calls_ended` (`tests/two_daemons.rs`), the same against a running daemon |
