...
6. **P9-2** The faster start from the connect frames' SNR, with its bench number. ~~And the
   faster climb~~ — done 2026-09-16 (ADR-0007): a learned margin is given back at an
   accelerating rate once clean burst follows clean burst (1.5 dB in seven bursts, not
   eighteen); model first, port bit-exact on the regenerated rate traces; +2 % to +5 % net on
   the link benches with no point worse than −3.4 %. Found on the VarAC bench, where one
   collision held a 16 kB transfer a mode and a half below what 12 dB carried. Not taken:
   reading a lone failure as an accident (−11 % on Moderate at 16 dB), and `burst_frames`
   6 → 12 (44 acknowledgement exchanges for 208 frames were a fifth of the air time — its
   own lever, with its own curve).
