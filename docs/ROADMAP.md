...
4. **P7-0d** VarAC through the adapter at 500 Hz over `[sim]` (two daemons at
   ...
   the air with a VarAC station. VarAC's ping is a short session over `CONNECT`, so it
   does not wait on P7-1.
5. **P7-1** The link probe: ~~frame format (ADR-0006), model~~ — done 2026-09-15; the
   port, `probe` in the control API, a Probe button beside Beacon, both SNRs on the
   dashboard. No `PING` in the VARA adapter (VARA has none; VarAC pings by connecting).
