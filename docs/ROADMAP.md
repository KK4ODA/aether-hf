...
| P3-2 🔄 | Port PHY + frame codec + ARQ to core, cross-validated against the model. Done: waveform, constellations, mode table, frame codec, OFDM, preamble, transmitter (`aether-phy`); link frame formats and rate control (`aether-link`). the receiver and acquisition (`aether-phy` is complete: a frame can be built, found in a stream, demodulated and decoded entirely in Rust); link frame formats and rate control (`aether-link`). Remaining: the ARQ engine | P3-1 |
