...
**Version:** 0.1 (2026-09-13), revised to v0.2.0-beta.68 (2026-09-26) · **Companion document:** [`AUDIT.md`](AUDIT.md) (detailed evidence)
...
*The assessment at the audit, 2026-09-13, kept as the starting point; §13 records what has been
done since.*
...
## 2. Architecture at the audit (2026-09-13)
...
flowchart TB
  ...
  subgraph API["Application-facing adapters (PHY-agnostic)"]
    ...
    KISS["KISS port (HF, ADR-0019)<br/>AGWPE (FM phase)"]:::reuse
...
`aetherd`; as built: `aether-fec`, `aether-phy`, `aether-link` and `aetherd`, which holds the
audio, the keying and both network surfaces) with a **Tauri v2** desktop shell. Port
module-by-module against golden vectors
...
aether-hf/
  ...
  app/             Tauri desktop app (plain ES modules, no build step — ADR-0005; the daemon
...
| Low-SNR modes | BPSK R=1/5; BPSK R=1/5 + 4× spreading ("robust") | honest floor: ≈ −5 dB (3 kHz, Poor) at ~300 bps, ≈ −8 to −10 dB at ~80 bps. Drop the DSSS/FSK "emergency" concept. *Superseded: the floor is a
steady-envelope 16-FSK family (ADR-0013; rungs 0–1 of both ladders, 36–54 bit/s, 2 300 Hz
sessions to −14 dB) — FSK as the floor, not an emergency mode* |
...
- **Datagram service:** beacon / CQ / ping (link-quality report) / UI frames — needed for
  VarAC-style workflows, busy-channel etiquette, and KISS/AX.25 — built on HF as datagrams
  (ADR-0019, beta.61).
...
ecosystem uses them for VARA-class modems); the KISS port is TCP, and a serial-only client
goes through `socat` (`docs/user/kiss.md`); revisit for FM.
...
- Branching: trunk-based — `master` under a ruleset (no deletion or force-push; pull requests
  need CI green), short-lived branches, **rebase merges only, never squash**: git-cliff writes
  the release notes from the commits (`docs/MAINTAINING.md`). Releases cut from tags; hotfix
  branches from tags when needed.
...
4. **Datagram service in the link layer** so an FM PHY can carry AX.25/KISS UI frames —
   built on HF as the `DATAGRAM` kind (ADR-0019); an FM PHY reuses it unchanged.
5. **Separate crates** (`aether-phy`, later `aether-phy-fm`) with identical test
...
| Feature | Value | Phase | State (2026-09-26) |
|---|---|---|---|
| Headless daemon + web UI (already implied by §4.2) | remote stations, Raspberry Pi gateways, RMS operators | 3–4 | ✅ P3-3, P4-1 |
| Built-in channel simulator as an audio backend | test Winlink/Pat end-to-end with no radio; demo mode; CI | 1 | ✅ `[sim]`, P6-3 |
| Session audio capture + replay (`.wav` + JSON metadata) | every field failure becomes a regression test | 1 | ✅ P6-1, P6-2 |
| Standard diagnostic bundle | supportable software | 4 | ✅ P4-5 |
| Link-quality ping / beacon with SNR report both ways | VarAC-style QSO tools, propagation checks | 3 | ✅ the probe (ADR-0006), every control frame (ADR-0021), beacons on a timer (beta.67) |
| Busy-channel detector (energy + structure) | etiquette, required by Winlink Express (`BUSY`) | 3 | ✅ P3-3; the passband's shape since beta.39 |
| CW ID option and clear-text callsigns in headers | regulatory compliance, no-argument identification | 3 | ✅ P3-6, ADR-0017, ADR-0022; at most 20 wpm under the US profile |
| Digirig / CM108 / IC-7300-class auto-detection profiles | zero-config for the most common hardware | 4 | ✅ P4-4; CM108 keying beta.23; the Icom CAT preset beta.62 |
| Portable profiles (JSON) | club stations, emcomm go-kits | 4 | ✅ ADR-0011, beta.43 |
| Winlink gateway mode notes (RMS Trimode compatibility) | more Aether-capable gateways = more usable network | 3/6 | ✅ P3-8; the Trimode bench still owed |
| PAPR-aware "average power" meter | operators see the real advantage of drive settings | 2 | open: the panel shows the last peak |
| Spectrum occupancy log | choosing frequencies; emcomm planning | 4+ | open |
| Modem-to-modem telemetry (SNR both ways in ACKs) | already part of ARQ design; expose in UI | 2 | ✅ `peer_snr_db` (beta.14); every control frame since ADR-0021 |
| Public gateway registry + beacon/CQ discovery frames | answers "where are the RMSs?" — the top adoption concern in the Mercury thread | 3 | frames ✅; the registry open |
| Packaging for ham Linux distros (`.deb`, ARM64, LiaisonOS/DragonOS inclusion) | gateways run on Pis; distros are a free distribution channel | 3–5 | `.deb`, AppImage and the aarch64 daemon ship; distro inclusion open |

Not recommended now: encryption (illegal on amateur bands in most jurisdictions), mesh/relay.
(A phone application is Phase 8, on the back burner.)

---

## 13. Prioritized implementation roadmap

Dependencies are listed by task ID. "Files" in Phase 0 are the layout before the move to
`model/`.
...
| P2-3 ✅ (acquisition) | Low-SNR acquisition: PMF-FFT matched-filter bank, type-by-preamble-sequence, chip-signalled mode (ADR-0002 amendment) — 100 % at −5 dB, 87 % at −7 dB. Since: below −6 dB, ADR-0009's floor and then the tone floor (P9-8, ADR-0013: 2 300 Hz sessions to −14 dB); connect probability, `tools/bench_calls.py` (`calls.csv`, ADR-0016) | P1-1, P2-1 |
...
as a service for 72 h unattended. Status: over the simulator, met (P6-6, P7-0d); over a
virtual audio cable and the 72 h Pi run, not yet recorded.
...
| P4-7 🔁 | **Acceptance.** Open, and not something this repository can close on its own: a new user installing and completing a Pat/Winlink session with only the wizard, and a usability test with three external hams. Done since: CM108 keying (beta.23, untested on hardware), real icons (`tools/make_icons.py`, beta.9), the daemon and panel packaged with the shell (P5-2). Open: a new user's Pat/Winlink session with only the wizard — Setup's numbered flow since beta.10 — and three external hams (W4TGA, ND1J and KE4QCM have run betas on the air, which is not the usability test) | P4-4, P3-5 |
| P4-8 ✅ | **Profiles** (ADR-0011, beta.43): a station's settings as one portable file under a name (`profiles/<name>.aetherprofile`), projected through a settings registry that states every key's scope, bounds and reason (`settings.rs`, `config.schema`); applied key by key with a report (unknown, ignored, invalid, missing hardware — suggested, never chosen); `profile.*` and the Profile bar at the top of Setup | P4-2 |
| P4-9 ✅ | **The panel since P4-1.** The instrument look and artwork (beta.9); Setup as one numbered flow, which a first run opens on (betas .10, .25); the dashboard, Stations and Diagnostics tabs, Compact (beta.14); dial memories, Tune, and a tooltip on every control (beta.26, a rule since); the session banner and chime (beta.46); the Test's progress, the session history and the question before closing a busy modem (beta.58); the Send box — time, ✓ once the other station has all of it (beta.59); the design system, the undockable signal card, Help / About, the rules badge (beta.60); KISS programs in Setup (beta.61); every tab a console — boxes titled inside, help behind a "?", the main thing given the spare height (betas .65, .67); unsaved Setup changes said on every tab, and *Stop calling* (beta.68) | P4-1…P4-6 |
...
| P5-3 ✅ | **The release pipeline** (`.github/workflows/release.yml`): from a tag, the daemon for x86_64 and aarch64 Linux (built on Ubuntu 22.04 so it runs on a Raspberry Pi's Debian 12) and Windows, the desktop application for Windows, Linux and — since beta.22, unsigned — macOS on Apple Silicon (`dmg` and the updater bundle), the daemon for macOS too, an SPDX SBOM, SHA-256 sums, and release notes from Conventional Commits (`cliff.toml`). `vX.Y.Z` is stable, `vX.Y.Z-beta.N` a prerelease, and a nightly runs on a schedule from the default branch into one rolling prerelease. Signed for the updater when `TAURI_SIGNING_PRIVATE_KEY` is set; a branch push saying `[dry-run]` builds everything without publishing (two dry runs green on all five targets) | P5-1, P5-2 |
| P5-4 ✅ | **Settings migration.** The configuration names its schema; an older file is brought forward through an ordered chain on load, backed up beside itself first, and rewritten; a newer file is never read with keys dropped: the daemon starts from the newest copy it can read (`station.toml.bak-v<n>`), keeps the newer file as `.newer-v<m>` and says so (`status.config_note`, beta.57), and refuses only when there is none. `core/aetherd/tests/data/config/` holds files as each released version wrote them, and a test loads every one and checks no key or value is lost — the upgrade promise CI demonstrates from now on | — |
| P5-5 ✅ | **Signed updates and a way back.** The shell checks on start (on the channel `[update] channel` names, set from the panel) and asks before installing; stable never sees a prerelease; every installer it installs is kept locally and *Help > Restore the previous version* runs the kept one without a network. The Ed25519 public key is in `tauri.conf.json`; the private key is on the maintainer's machine and in the `TAURI_SIGNING_PRIVATE_KEY` secret — every release from beta.2 is signed. Since: the updates window (beta.14); an unset channel follows the build, so a beta follows the betas (beta.56; configuration schema 5 turned a beta's `stable` into `beta` once); a restore across a configuration schema puts back the settings the older version reads (`SCHEMA_HISTORY`, beta.57); the uninstaller asks before removing settings, profiles, logs or recordings, keeping them by default (beta.58); Help / About shows the updater and runs its check (beta.60) | P5-3 |
...
| P5-7 🔁 | **Open.** Windows Authenticode through an open-source signing service (`docs/user/install.md` explains SmartScreen meanwhile); Apple signing and notarization (needs a developer account; the `dmg` ships unsigned, untested on a Mac with a radio); the first stable `vX.Y.Z` — every release so far is a `-beta.N` pre-release, so `releases/latest` stays empty by design; an MSI if anyone asks; §9's post-install self-test (not built). Done: the first tagged release (beta.2, 2026-09-14, signed), real icons (beta.9) | P5-5 |

Acceptance as written asks for an N−1 → N upgrade and a forced-failure rollback demonstrated
in CI. The upgrade half is the fixture test in P5-4, with files as released versions wrote
them (beta.53, .55, .60, .61); going back across a schema is the shell's restore (beta.57). The rollback half is exercised by hand — the kept-installer path is a native
...
| P6-6 🔁 | **The air.** Audio cable → ground wave → NVIS → 500–2 000 km → RMS gateway trial; twenty sessions across three classes; recalibrate the simulator on the disagreements. Human, and not something this repository can do for itself. Bench row 0 is in `field/LOG.md`: the Pat session measured 686 bit/s against 1 059 predicted for the same session size with a real station's latency — the rest is B2F's own round trips, which the simulator's single transfer does not model. VarAC 15.0.18 was tried over `[sim]`: it talks to the adapter (three of its start-up commands were `WRONG` and are now heard; its `VERSION` parser wants three words, which the reply now is) but its ecosystem is 500 Hz — it disables its interface until `BW500` is `OK` and refuses to call at 2300 Hz on a calling frequency — so VarAC needs a 500 Hz waveform — P7-0, with which VarAC pings and connects over `[sim]` (P7-0d, beta.18); on the air, not yet. **Winlink Express 1.8.5.0 passes the bench too**: two instances over `[sim]`, a P2P message with a 6 kB incompressible attachment byte-identical on arrival, 854 bit/s measured against 1 049 predicted (0.81). It found the bug the Pat bench could not: Pat's `MYCALL` happened to match the daemons' configured callsigns, Winlink Express's did not, and `MYCALL` had never reached the modem — now it does, in the model and the core (`callsigns.set`, `connect` with `callsign`). Both benches are rows in `field/LOG.md`. **The first radio-to-radio test ran 2026-09-17** (40 m, 500 Hz, mobile ↔ home): the link established and carried data both ways, but held a clean session only once in six — a compression-stream desync on the lossy path (compression off by default since beta.28; an integrity check is open) is the finding — the large frequency offset on the mobile's frames proved a noise-trigger artefact (beta.27 stops reporting it) — with the action list and the next-test protocol in `field/OTA-FOLLOWUPS.md`. **The ND1J Test sessions (2026-09-25, ND1J in Senoia ↔ KK4ODA near Northlake Mall, Atlanta; 40 m and 80 m)** found the tone bursts overrunning the key watchdog — the cut frame pinned the link to the floor — and the Test's ladder never reached: ADR-0017 (beta.58). On the air since (analyses in `field/` and the ADRs): the FTDX10's scope, 2026-09-14 — the key released before the card's last quarter second, retries in pairs (betas .7, .8); an idle band through the rig's AGC (beta.17); OTA-2, 300 ft on 40 m, 2026-09-17/18 (`field/OTA-2-FINDINGS.md`: control-frame confidence and the sent-frame record, beta.32; the busy detector reworked, betas .35–.41); the scope during *Set drive*, 2026-09-19 (a hole early in every burst: ADR-0010, beta.42); the first 10-mile test and W4TGA, 2026-09-23 (a spur latching busy, the receiver's bank rows computed once, the session banner and chime: beta.46; the weak-signal plan); ND1J's 7.082 MHz Test and chat, 2026-09-25 (ADR-0020, beta.63; ADR-0022, beta.66); seven sessions with KE4QCM on 7.082 and 3.590 MHz, 2026-09-25 (ADR-0023, beta.68). The count: 22 air rows in `field/LOG.md` beside three bench rows — the 2026-09-17 mobile test, and 21 sessions with W4TGA, KE4QCM and ND1J from 2026-09-23 to 26 folded in with `field_ingest.py` on 2026-09-26, classed by the author (Good, Moderate, Poor: all three classes). Still owed: the simulator-against-air comparison (`tools/compare_air.py`) for the 20 % criterion, and recordings in `field/sessions/`, which holds none, so the field tier passes empty | P6-1…P6-5 |
| P6-7 🔁 | **On-air crowdsourcing.** *2026-09-16: (b), (c) and (d) built in one commit (beta.23) — the Test session, `[operator]` in Setup and every sidecar, the path length, `pin_mode` and stranded-frame re-encoding (model first), the Contribute link, `tools/field_ingest.py` and `bench_link.py --replay`, which has since priced a real path (ADR-0020: ≈ 350 bit/s where the link ran 73–86). The loopback's short 64-QAM rungs (OFDM modes 12, 13; rungs 18–19 now) remain P9-1's. 2026-09-25: the Test runs probe → call → message → **ladder** → file → disconnect, the message sized from how the other station hears this one (ADR-0017, beta.58), and its pinned rungs no longer teach the receiver's controller (ADR-0020, beta.63). Open: (a) as amended, (e), and the volunteers' sidecars — ND1J's Tests are not yet in `field/paths.csv`.* The one thing no bench can give is how each mode behaves on real paths, per channel class, and what the receiver's SNR estimate means there — so every volunteer contact should leave a sidecar the bench can replay, with no effort from the volunteer. (a) The author's station unattended — `[regulatory] control = "automatic"`, `answer_only`, auto-recording on — at agreed times inside a §97.221(b) sub-band (the frequency plan's 3.590, 7.101, 10.141 (500 Hz), 14.107 MHz …) or on 6 m (50.690 MHz). Not on a 500 Hz calling frequency outside them — every Aether emission is over 500 Hz by the wider reading (ADR-0018), so §97.221(c)'s answer is not available — and no beacon schedule: `beacon.every` is refused under automatic control (§97.203(d)) and on answer-only. (b) A **Test session** — one control-API method and a panel button — that runs a fixed sequence and records it: Probe (both directions' SNR, ADR-0006), a 2 kB message, a 16 kB file, and a **mode ladder** of short bursts pinned at each mode from the floor up until three fail: real FER against measured SNR per mode on a real path; the sidecar gains the volunteer's grid, rig, power and antenna, and the path length from the two grids. (c) A submission path with no infrastructure at first: the diagnostics bundle grows a *Contribute this session* action (sidecar always, audio opt-in) that opens a pre-filled GitHub issue or an email; `tools/field_ingest.py` folds submissions into `field/LOG.md` and a per-path CSV; a hosted collector only when the volume asks for it. (d) The use: `bench_link --replay <sidecar>` runs the model's engines against the recorded SNR trace and failure pattern, so every controller change is judged on real paths; per-class penalties fitted from real ladders (the on-air equivalent of `channel_thresholds`); the SNR estimator checked against what the ladder decoded; acquisition misses and false-busy on real noise; and the open question of ADR-0008 (a 64-QAM first burst on a clean loopback) answered with hundreds of first bursts. (e) Honest and safe: volunteers run the beta through the wizard (which is also the three-hams item), the sequence sends only test bytes, and the aggregated curves and a heard map are published back to them | P6-6, P7-1 |

### Priorities from 2026-09-15 — done or overtaken

*Overtaken 2026-09-23 by the weak-signal plan below, and since 2026-09-24 by field testing of
the daemon as it is.*
...
| Order | Item | Why now |
|---|---|---|
| 1 | ~~**P7-0** the 500 Hz waveform~~ | done 2026-09-15 (betas .15, .18) |
| 2 | ~~**P7-1** the link probe~~ | done 2026-09-15 (ADR-0006, beta.16) |
| 3 | ~~**P9-2** a faster start from the connect frames' SNR~~ | done 2026-09-16 (ADR-0007, ADR-0008, beta.19) |
| 4 | ~~**P9-4** modes below 200 bit/s~~ | done 2026-09-16 (ADR-0009, beta.20); superseded 2026-09-24 by the tone floor in both bandwidths (ADR-0013, beta.52) |
| 5 | **P9-1** the A/B bench against registered VARA at the audio level | tool built 2026-09-16 (beta.24); `bench/ab/results.csv` still empty — the author's runs |
| 6 | **P9-3** sparser pilots, a short prefix, 2 750 Hz | open; after the P9-1 runs |
| 7 | ~~**P9-5** time diversity~~ | paused 2026-09-24 (an experimental opt-in if built) |
...
| 6 | **P9-1** the A/B against VARA through the channel cable | the tool is built (2026-09-16, beta.24) and P9-8 is in; `bench/ab/results.csv` holds no run yet — the author's runs, four virtual cables |
...
frames, that small step joins P9-7. P9-8 shipped in beta.52 and took ADR-0009's OFDM floor off
the 500 Hz air; weak paths use the tone floor in either bandwidth.

---

### Phase 7 — The 500 Hz waveform and the link probe (done 2026-09-15, betas .15–.18)
...
### Phase 9 — The modem's second rung (in progress, in the weak-signal plan's order: P9-2, P9-4, P9-6…P9-11 done — P9-4 superseded by P9-8 —; P9-5 paused; P9-1's runs and P9-3 open; P9-12 an option)
...
| P9-8 ✅ | **The tone floor** (ADR-0013) — done 2026-09-24, model then port: 16-FSK at 25 Hz, 40 ms symbols, a constant envelope at the OFDM frames' peak, three Costas sync blocks naming kind and RV; `tone-24` and `tone-36` are rungs 0–1 of both ladders and `tone-control` the floor's control frame. On the air (ND1J, 2026-09-24/25) six 5.36 s tone frames made a 32 s burst against the 30 s key watchdog; the cut frame counted as a failure and held the link on the floor from beta.52 until bursts fit the key (ADR-0017, beta.58). What was planned: a steady-envelope multi-tone FSK frame family, the same in both bandwidths, carrying calls, answers, acknowledgements and the slowest data — tens of bit/s to about 200. Designed from public sources: MIL-STD-188-141's 8-FSK automatic link establishment, the published FT8/FT4 design and its synchronisation on a fixed tone pattern, and textbook noncoherent FSK detection with soft decisions into Aether's LDPC. A call in the tone floor is heard by a station of either bandwidth, which then negotiates. Model, then the equal-peak-power curves (the gate in the plan above), then the link layer, then the port within the receiver's CPU budget on a modest PC, then a beta and the air | P9-6 |
...
| P9-12 (option) | **A spread-tone floor** — an option, not scheduled (added 2026-09-24 at the author's request). The tone floor keeps its sixteen tones inside 400 Hz, which is what lets a constant envelope carry the transmitter's whole peak power, and also what puts the whole frame inside one fade on a frequency-selective path — a two-ray path has a notch every 1/τ: 2 kHz on ITU Good, 1 kHz on Moderate, 500 Hz on Poor — and under any carrier parked on it. Since beta.55 every call, probe and beacon starts there, with a call's OFDM tries the only other way through (ADR-0016). The author's impression from the waterfall is that VARA's slower levels fill the passband. The option, on the 2 300 Hz air only (500 Hz has no room): the same frame with its tones spread across the passband, still one tone at a time with continuous phase, so the envelope stays constant and a notch or a carrier costs the symbols on its tones instead of the whole frame. Candidates, from public designs: wider tone spacing (MIL-STD-188-141's ALE spaces its eight tones 250 Hz apart from 750 to 2 500 Hz); the 400 Hz tone set hopped across sub-bands on a known pattern, as the sync blocks' Costas patterns are known; each symbol sent in two sub-bands at half the rate. **What it would cost:** glides between distant tones (a longer glide, or splatter); a floor that is no longer the same frame on both airs, so the two bandwidths stop hearing each other's beacons unless a receiver looks for both floors; the detector's work; a link-protocol version if it replaces the 400 Hz floor at 2 300 Hz; the KISS datagrams, sent on the floor because both bandwidths decode it (ADR-0019 §2.1); and the narrow floor near a segment edge, where the gate holds a link to the rungs that fit (ADR-0018). **Evidence first:** beta.55 recordings in which a call's floor tries fail while its OFDM tries get through, or floor frames are lost at an SNR that should carry them (`bench_link.py --replay`). **Gate**, as P9-9's: at equal peak power and through the detector, 2 dB or more over the 400 Hz floor at the same rate on Moderate and Poor and nothing worse on AWGN and Good (`bench_tone.py`), then sessions on the fading pipe with the spread frame's own shape (`fading.py`) | P9-8, P9-11 |
...
single-carrier QPSK/GMSK waveform; an AGW adapter beside the KISS port HF already has
(ADR-0019); reuse everything else (§11).
...
7. **P6-7** On-air crowdsourcing — *(b)–(d) built 2026-09-16 (beta.23) — the Test session, the
   Contribute link, `field_ingest.py`, `bench_link --replay`; reordered by ADR-0017 (beta.58):
   probe → call → message → ladder → file. Open: (a) inside the rules, (e), the volunteers'
   sidecars folded in*: the Test session (probe, 2 kB, 16 kB, the mode ladder)
...
8. ~~**P9-4** The floor~~ — done 2026-09-16 (ADR-0009): the floor frame family at 500 Hz,
   ...
   superseded 2026-09-24 by the tone floor (ADR-0013, beta.52).
9. **P9-1** ~~`tools/channel_cable.py` and the A/B protocol~~ (built 2026-09-16); the runs when the author can.
10. **P9-3**, with its curve on Good, Moderate and Poor, after the P9-1 runs; **P9-5** paused
    2026-09-24 (item 12).
...
12. **The weak-signal plan** (from 2026-09-23; its section above Phase 7 has the evidence and
    ...
    ADR-0013, beta.52; on the air since 2026-09-24/25 — ADR-0017), ~~**P9-9**~~ (done 2026-09-24:
    fast tones, ADR-0014, beta.53; on the air on 80 m, rung 3 open — ADR-0017 §4),
    ~~**P9-10**~~ (done 2026-09-24: the 500 Hz middle kinds, ADR-0015, beta.54; on the air in
    ND1J's 500 Hz ladder — ADR-0020),
...
13. **Regulatory compliance** (done 2026-09-25, ADR-0018, beta.60; configuration schema 6): every transmission is judged
    ...
    automatic control under §97.221, 60 m — and 6 m since beta.68: 50.1–54.0 MHz data,
    Technician and up, all of it an automatic-control segment, 2.8 kHz under §97.307(f)(2),
    (5); the dial list proposes 50.690 MHz) — from the spectrum each waveform measures; the
    link is capped to the rungs the rules allow; the control mode is the operator's to say.
    Owed: another administration's profile when somebody asks for one; an operator's report of
    LEGAL / WARNING / TX BLOCKED on a real dial — the gate has been in the path of ND1J's and
    KE4QCM's sessions on 7.082 and 3.590 MHz since beta.60. The panel's redesign (the
    ...
    as one entry a row with filters, and Help / About with the updater's status — reworked as
    consoles in betas .65 and .67 (P4-9).
14. **The KISS port** (done 2026-09-25, ADR-0019, beta.61; configuration schema 7): programs set up for VARA HF's KISS port —
...
15. **Rate control on a real path** (done 2026-09-25, ADR-0020, beta.63): from ND1J's 7.082 MHz Test,
    ...
    not the Test ladder's pinned rungs. Owed: a Test on a workable path with this build —
    KE4QCM's sessions on betas .63–.65 ran where data seldom decoded (ADR-0023); the link bench's
    pipe decoding retransmissions as the PHY does (P9-6); whether 40 m wants a gentler margin
    step.
16. **A burst fits the key; a session ends on the air** (done 2026-09-25, ADR-0017, beta.58;
    ND1J's Tests): a burst carries only what fits `max_key_s` less lead, tail and a second — a
    tone burst is five frames — and the identifier when it rides; Abort cuts the burst; every
    session's end is identified once; a stopping daemon ends the session on the air, and the
    shell asks before closing; the Test climbs its ladder before the file. With it: the session
    history (`sessions.json`, `sessions.list`), the Sent pane — the Send box in beta.59, with ✓
    once the other station has all of it (`send {data, ref}`, the `sent` event,
    `tx_undelivered_bytes`, model first) — and the uninstaller's questions. Open: the 80 m
    rung-3 failures at ≈ 0 dB.
17. **Heard there, both ways** (done 2026-09-25, ADR-0021, beta.64): every control frame but an
    acknowledgement carries the SNR of the last frame its sender decoded from the other
    station; a station that only received learns from the caller's DISC how it was heard
    (`ended_peer_snr_db`, which the history takes first), and the history says *none sent* /
    *not said* where a direction had no traffic.
18. **The end of a session waits out the identifier** (done 2026-09-25, ADR-0022, beta.66;
    ND1J's chat on 7.082 MHz): a leaving station takes no data frames and moves its DISC retry
    past a frame heard arriving; a DISC_ACK is a response and never waits for a clear channel;
    DISC, DISC_ACK and the closing identifier wait out the other station's Morse ID (≤ 15 s); an
    identifier appended to a burst moves the engine's timers. Open: the ten-minute identifier
    mid-session (a frame field; a protocol change).
19. **Beacons on a timer, and the identifier's speed said** (done 2026-09-25, beta.67; ND1J's
    questions): `beacon.every {minutes}` (10–240, runtime only; each waits for an idle station
    and a clear channel and is skipped, not caught up, after two minutes; refused under
    automatic control — §97.203(d) — and on answer-only); beacons logged sent and heard and
    counted per station (the Stations tab's Beacons filter), `status.beacon`; the Morse
    identifier's speed as sent — at most 20 wpm under the US profile (§97.119(b)(1)) — in
    `status.identifier` and Setup, the `cw_id` keys live.
20. **Leaving, handing over, and 6 m** (done 2026-09-25, ADR-0023, beta.68; KE4QCM's sessions on
    7.082 and 3.590 MHz): a receiving station's Disconnect sends its DISC between bursts
    (`status.closing`; Disconnect reads *Stop calling* while a call is made); the TURN waits for
    the longest first frame — the tone floor's — and past a frame heard arriving; a called
    sender that hears a POLL yields and the caller keeps the turn; WANT_TX counts frames in
    flight. 6 m under the US profile (item 13). Owed: the air for both.
