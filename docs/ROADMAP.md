...
| P9-8 | **The tone floor** (ADR-0013). A steady-envelope multi-tone FSK frame family, the same in both bandwidths, carrying calls, answers, acknowledgements and the slowest data — tens of bit/s to about 200. Designed from public sources: MIL-STD-188-141's 8-FSK automatic link establishment, the published FT8/FT4 design and its synchronisation on a fixed tone pattern, and textbook noncoherent FSK detection with soft decisions into Aether's LDPC. A call in the tone floor is heard by a station of either bandwidth, which then negotiates. Model, then the equal-peak-power curves (the gate in the plan above), then the link layer, then the port within the receiver's CPU budget on a modest PC, then a beta and the air | P9-6 |
...
12. **The weak-signal plan** (from 2026-09-23; its section above Phase 7 has the evidence and
    ...
    2026-09-24: holding the link, ADR-0012), **P9-8** the tone floor, **P9-9** only if the curves ask for it, then **P9-5**
