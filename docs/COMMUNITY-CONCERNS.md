...
| 8 | **ALC spikes when transmission starts** ("high ALC spikes that dropped as data kept going — ugly, might cause issues"). | medium | PAPR study (P2-4); level calibration (§8) | **Gap.** Added a TX-envelope requirement to §5.2: preamble PAPR ≤ data PAPR, constant average power across preamble/data/ACK, soft ramp-up; wizard shows an ALC/peak meter. |
