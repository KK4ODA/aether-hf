...
from aether_model.speed_levels import BITS_PER_SYMBOL, WIDE_LEVELS, Modulation
...
def test_ofdm_symbol_survives_the_audio_path(rng: np.random.Generator) -> None:
    ...
    bits = rng.integers(0, 2, DATA_CARRIERS_W * 2).astype(np.int8)
    tx = mod.modulate(Mapper(Modulation.QPSK).map(bits))
    ...
    got = Demapper(Modulation.QPSK).hard_demap(OFDMDemodulator("wide").demodulate(rx))
...
def test_speed_table_rates_follow_from_the_waveform() -> None:
    ...
    for level in WIDE_LEVELS:
        if level.modulation in (Modulation.FSK2, Modulation.FSK4):
            continue
