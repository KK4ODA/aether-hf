...
from aether_model.link.rate import NARROW_AWGN_THRESHOLD_DB, NARROW_PAYLOAD_BYTES, usable_modes
...
def test_narrow_chip_set_is_its_own_and_the_wide_set_is_untouched() -> None:
    ...
    assert len(pre.sequences) == 4 * len(NARROW_MODES) == 40
...
def test_narrow_mode_table() -> None:
    ...
    # and a connect request fits the LONG layout at mode 0 (header, length and body)
    assert NARROW_MODES[0].payload_bytes(NARROW_LONG) >= CONNECT_BODY_BYTES + 5
    # every mode carries more than the one before it, or is a rate the table keeps for
    # its threshold; the fastest reaches a kilobit
    payloads = [m.payload_bytes(NARROW_LONG) for m in NARROW_MODES]
    assert payloads == sorted(payloads)
    assert payloads[0] == 25 and payloads[-1] == 137
    assert NARROW_MODES[-1].net_bit_rate(NARROW_LONG) == pytest.approx(1040, abs=1)
    # the rate controller's copies agree with the table
    assert {m.index: m.payload_bytes(NARROW_LONG) for m in NARROW_MODES} == NARROW_PAYLOAD_BYTES
    assert set(NARROW_AWGN_THRESHOLD_DB) == {m.index for m in NARROW_MODES}
    assert 0 in usable_modes(NARROW_AWGN_THRESHOLD_DB, NARROW_PAYLOAD_BYTES)
...
def test_narrow_floor_is_where_the_wide_floor_is(modem: Modem) -> None:
    """Mode 0 at −5 dB (3 kHz) with random offsets: the same SNR the wide table's BPSK ⅕
    is held to (``test_low_snr_acquisition_and_decode``), reached with QPSK ½ on a fifth
    of the bandwidth."""
...
def test_narrow_session_over_the_real_phy() -> None:
    ...
    assert a.rate.modes == usable_modes(NARROW_AWGN_THRESHOLD_DB, NARROW_PAYLOAD_BYTES)
