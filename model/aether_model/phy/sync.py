...
The OFDM floor family of ADR-0009 — eight-symbol preambles of its own sequences, an averaged
floor statistic, the contest between the two families — was retired with ADR-0013: below
the OFDM tables is the tone floor, found by :class:`aether_model.phy.tone.ToneDetector`.
...
class FrameSync:
    ...
    def detect_confidence(self, air: AirInterface) -> float:
        ...
        a noise trigger.
        """
        return self.timing_peak / max(air.acquisition_threshold, 1e-12)
...
class FrameDetector:
    def __init__(
        ...
        *,
        ...
        self._ref_segs: dict[FrameType, ComplexArray] = {}
        for ft in self._types:
            ...
            self._ref_segs[ft] = wave[: n_seg * SEGMENT_LEN].reshape(n_seg, SEGMENT_LEN)
        self.n_seg = self._ref_segs[FrameType.DATA].shape[0]
    ...
    def bank(self, x: ComplexArray, chunk: int = 8192) -> tuple[FloatArray, FloatArray, FloatArray]:
        ...
        for p0 in range(0, n_pos, chunk):
            ...
            for ti, ft in enumerate(self._types):
                spec = self._spectra(x, p0, p1, self._ref_segs[ft])
                mag = np.abs(spec)
    ...
    def fine_cfo(self, x: ComplexArray, start: int, frame_type: FrameType) -> float:
        ...
        per = self.period
        c2 = complex(np.vdot(y[:per], y[per : 2 * per]))
    ...
    def stream_lookback(self) -> int:
        ...
        so that every candidate is decided in a region that holds all it depends on: a
        symbol past the preamble."""
        return (PREAMBLE_SYMBOLS + 1) * self.period

    # ── full acquisition ──────────────────────────────────────────────

    def detect(self, x: ComplexArray, max_frames: int = 1) -> list[FrameSync]:
        """Find up to ``max_frames`` preambles in ``x`` (whole-buffer), earliest first."""
        x = np.asarray(x, dtype=np.complex128)
